...
// Placeholder demo catalog — swap for a real one when wiring this up to an
// actual merchant. Kept here just so get_cart has something to return.
const DEMO_CART = [
...
/**
 * Registers the Autopay checkout tools on an MCP server instance. Every
 * tool call is a real network call to the live Online v1.1 sandbox
 * (see AUTOPAY_SANDBOX_URL in lib/config.js) — the same hash-signing and
 * ITN-verification code paths a real integration would use, not a mock.
 */
function registerCheckoutTools(server) {
  server.registerTool(
    'get_cart',
    {
      ...
      description:
        'Zwraca przykładowy koszyk (pozycje, ilości, ceny, suma). Użyj przed initiate_payment, żeby znać kwotę do zapłaty.',
      inputSchema: {},
    },
    async () => {
